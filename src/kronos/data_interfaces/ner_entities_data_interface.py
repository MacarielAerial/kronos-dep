...
class NEREntity:
    ...
    def validate(self, parent_text: str) -> bool:
        ...
        if substring != self.text:
            raise ValueError(f"Entity text does not match the specified range "
...
def doc_to_ner_entities(doc: Doc) -> List[NEREntity]:
    return [NEREntity(text=ent.text,
...
def list_dict_to_ner_entities(list_dict: List[Dict[str, Any]]) -> List[NEREntity]:
    return [NEREntity(text=entity_dict["text"],
...
def list_text_to_list_ner_entities(list_text: List[str], spacy_pipeline: Language) -> List[List[NEREntity]]:
    list_ner_entities: List[List[NEREntity]] = []
    list_doc = list(spacy_pipeline.pipe(list_text, batch_size=32, n_process=cpu_count()))
    list_ner_entities = [doc_to_ner_entities(doc) for doc in list_doc]

    i_log: int = random.choice(range(len(list_ner_entities)))
    logger.info(f"{len(list_ner_entities)} NER entities found in "
